...
public class MemberController {
    ...
    @PostMapping("/login")
    public MemberDomain signIn(@RequestBody(required = false) MemberWrapper w) {
        return membersService.login(w);
    }

    @PostMapping("/register")
    public MemberDomain signUp(@RequestBody(required = false) MemberWrapper w) {
    ...
    public TransferPointDomain transferPoint(@RequestBody TransferPointWrapper transferPointWrapper, HttpServletRequest request) throws Exception {
        String id = request.getAttribute("id").toString();
        String payee = utilService.searchDatabaseID(transferPointWrapper.getPayee());
        if (id == null || payee == null) {
            throw new IllegalArgumentException("Invalid id or payee");
        }
        transferPointWrapper.setOriginID(id);
        transferPointWrapper.setPayee(payee);
    ...
    public void resetPassword(@RequestBody ResetPasswordWrapper resetPasswordWrapper, HttpServletRequest request) {
        ...
        membersService.setPassword(resetPasswordWrapper);
    }
